...
import { calcTotalPresupuestado, calcTotalGastosReal, calcDesvioPorcentual } from "@/lib/calculations";
...
const EMPTY: Omit<Gasto, "id"> = { concepto: "", categoria: "Otros", presupuestoEstimado: 0, costoReal: 0, estado: "Pendiente", proveedor: "", fechaPago: "", notas: "" };

export default function PresupuestoPage() {
  const { gastos, addGasto, updateGasto, deleteGasto } = useStore();
  ...
  const totalPresupuestado = calcTotalPresupuestado(gastos);
  const totalReal = calcTotalGastosReal(gastos);
  const desvio = calcDesvioPorcentual(gastos);
  const gastosConfirmados = gastos.filter((g) => g.estado === "Confirmado" || g.estado === "Pagado")
    ...
    <div className="flex flex-col flex-1">
      ...
      <div className="p-4 md:p-6 space-y-4 md:space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <KPICard title="Total Presupuestado" value={formatCurrency(totalPresupuestado)} subtitle="Suma de estimaciones" icon={Wallet} accent="cyan" />
          <KPICard title="Total Real" value={formatCurrency(totalReal)} subtitle="Costos confirmados/pagados" icon={TrendingDown} accent={totalReal > totalPresupuestado ? "red" : "emerald"} />
          <KPICard title="Confirmado + Pagado" value={formatCurrency(gastosConfirmados)} subtitle="Comprometido firmemente" icon={CheckCircle2} accent="blue" />
          <div className={cn("rounded-xl border p-5 flex flex-col gap-3 bg-white dark:bg-slate-900",
            ...
            <p className="text-slate-400 dark:text-slate-500 text-xs">{totalReal > 0 ? `${formatCurrency(Math.abs(totalReal - totalPresupuestado))} de diferencia` : "Sin costos reales cargados"}</p>
          </div>
        </div>
      ...
      <GastoDialog open={dialogOpen} onOpenChange={setDialogOpen} initial={editingGasto || undefined} defaultValues={EMPTY} onSave={handleSave} />
    </div>
  );
}
